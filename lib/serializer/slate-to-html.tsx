...
let tableinfo: TableInfoType = { id: 0, colSizes: [] };

export const getTableInfo = () => {
  let styles = "";
  tableinfo.colSizes.forEach((size, index) => {
    size > 0 &&
      (styles += `.${tableinfo.id} td:nth-child(${
        index + 1
      }) { width:${size}px; }`);
  });
  return styles;
...
const setTableInfo = (info: TableInfoType) => {
  tableinfo = info;
};
...
const getstyleFormat = (element: Descendant & any) => {
  ...
  if (!isStyle) return "";
  return `style="${
    element.indent ? `margin-left: ${24 * element.indent}px;` : ""
  }${element.align ? `text-align: ${element.align};` : ""}${
    element.lineHeight ? `line-height: ${element.lineHeight};` : ""
  }${
    element.listStyleType ? `list-style-type: ${element.listStyleType};` : ""
  }"`;
};
...
const getNode = ({
  ...
  switch (element.type) {
    ...
    case "a":
      return `<a ${getstyleFormat(element)} href="${escapeHtml(
        element.url
      )}">${children}</a>`;
    case "h1":
    ...
    case "table": {
      if (element.colSizes)
        setTableInfo({ id: element.id as any, colSizes: element.colSizes });
      return `<table class="${
        element.id
      }" style="border-collapse:collapse;" ${getstyleFormat(
        element
      )}>${children}</table>`;
    }
    case "tr":
      return `<tr style="border-collapse:collapse;" ${getstyleFormat(
        element
      )}>${children}</tr>`;
    case "td":
      return `<td style="border-collapse:collapse; ${
        element?.borders ? ";" : "border: 1px solid black;"
      } ${
        element?.borders?.bottom
          ? "border-bottom: " + element.borders.bottom.size + "px solid black;"
          : "border-bottom: 1px solid black;"
      }${
        element?.borders?.top
          ? "border-top: " + element.borders.top.size + "px solid black;"
          : "border-top: 1px solid black;"
      }${
        element?.borders?.left
          ? "border-left: " + element.borders.left.size + "px solid black;"
          : "border-left: 1px solid black;;"
      }${
        element?.borders?.right
          ? "border-right: " + element.borders.right.size + "px solid black;"
          : "border-right: 1px solid black;;"
      }" ${getstyleFormat(element)}>${children}</td>`;
    case "img":
      const caption =
        element.caption && element.caption[0] && element.caption[0].text;
      return `<figure>
      <img width="${element.width ? element.width : "100%"}" src="${escapeHtml(
        element.url
      )}">
...
const getLeaf = ({ leaf, children }: { leaf: Leaf; children: string }) => {
  ...
  if (leaf.lineHeight) {
    newChildren = `<span style="line-height: ${leaf.lineHeight};">${newChildren}</span>`;
  }
  if (leaf.listStyleType) {
    newChildren = `<span style="list-style-type: ${leaf.listStyleType};">${newChildren}</span>`;
  }
  return newChildren;
};
